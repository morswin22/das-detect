...
def get_mask(shape, div):
    ...
    axes = (int(center_col / div), int(center_row / div))
...
def create_frequency_mask(shape, freq_min, freq_max):
    ...
    row_scale = rows / max(rows, cols)
    col_scale = cols / max(rows, cols)
                               
    Y, X = np.ogrid[:rows, :cols]
    dist_from_center = np.sqrt(((X - center_col) * col_scale)**2 + ((Y - center_row) * row_scale)**2)
...
show_gray(np.concatenate([og_img, low_pass_img, high_pass_img, both_pass_img], 1))
...
show_gray((np.concatenate([og_img, low_pass_fshift_img, high_pass_fshift_img, both_pass_fshift_img], 1)))
...
def downsample_image(img, target_shape):
    """
    Downsample the input image to the target shape.

    Args:
        img: The input image (2D numpy array).
        target_shape: Tuple (rows, cols) for the desired shape.

    Returns:
        downsampled_img: The downsampled image.
    """
    downsampled_img = cv2.resize(img, target_shape, interpolation=cv2.INTER_AREA)
    return downsampled_img

def upsample_image(img, original_shape):
    """
    Upsample the input image back to the original shape.

    Args:
        img: The input image (2D numpy array).
        original_shape: Tuple (rows, cols) for the original shape.

    Returns:
        upsampled_img: The upsampled image.
    """
    upsampled_img = cv2.resize(img, original_shape[::-1], interpolation=cv2.INTER_LINEAR)
    return upsampled_img

# Example: Downsample to square
downsample_shape = (512, 512)  # Target downsampled shape

print(len(img[0]))
img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
downsampled_img = downsample_image(img_gray, downsample_shape)

# Perform FFT on downsampled image
fshift, spectrum = fft(downsampled_img)

# Apply a frequency mask
low_pass_mask = create_frequency_mask(downsampled_img.shape, 1, 30)
filtered_fshift = fshift * low_pass_mask

# Perform inverse FFT on the filtered frequency spectrum
filtered_downsampled_img = ifft(filtered_fshift)

# Upsample back to the original shape
img_filtered = upsample_image(filtered_downsampled_img, img.shape).clip(0, 255).astype(np.uint8)

display(PIL.Image.fromarray(((spectrum * low_pass_mask)).clip(0, 255).astype("uint8")))
display(PIL.Image.fromarray(((spectrum)).clip(0, 255).astype("uint8")))
imshow(img_filtered)
